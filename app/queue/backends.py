...
class PostgresQueueBackend:
    ...
    def fetch(self, db, limit: int = 10) -> list[QueueEnvelope]:
        jobs = db.query(JobQueue).filter(JobQueue.status == "queued").limit(limit).all()
        return [
            QueueEnvelope(
                id=str(job.id),
    ...
    def health(self, db) -> dict[str, Any]:
        ...
        return {"backend": self.name, "available": True, "pending": pending, "failed": failed}
...
class RedisQueueBackend:
    ...
    def health(self, db) -> dict[str, Any]:
        client = self._client()
        client.ping()
        return {"backend": self.name, "available": True, "pending": int(client.llen(REDIS_QUEUE_KEY)), "failed": None}
