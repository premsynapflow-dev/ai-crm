...
- Billing: Razorpay for payments; Resend for transactional email
...
2. `FeatureGateMiddleware` — Feature flagging per client; single DB session per gated route
3. `SecurityHeadersMiddleware` — CSP, X-Frame-Options, etc.
4. `DatabaseRateLimitMiddleware` — In-memory sliding window rate limiting
5. `RequestAuditMiddleware` — Fire-and-forget audit writes via thread executor; skips static paths
...
- `record_metric()` is non-blocking — buffers in memory, background worker flushes every 30s
